...
| `mutation_test.py` | runs representative end-to-end mutations in a scratch worktree and requires the checker that owns the rule — `check_gates.py`, or `check_metrics.py` for the three AUPR contract cases — to fail; regex-level self-tests inside `check_gates.py` separately exercise every regression pattern | no (needs `git`) |
...
- `mutation_test cases=13 failures=0` — all 13 representative end-to-end injected regressions were
  detected: ten against `check_gates.py` (markers, an invariant, regression wordings, path hygiene) and
  three against `check_metrics.py`'s AUPR contract guard (Average-Precision convention, and each
  correctness score orientation). This is not a claim that every individual marker or invariant has its
  own worktree mutation; every regression regex is additionally exercised by `check_gates.py`'s synthetic
  self-test.
