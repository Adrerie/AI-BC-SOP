...
| The two-layer/binary conditions on the source's bound scope that construction, not certified robustness as a field; the family actually used must state its own assumptions | source-derived (the limits) + synthesized (the scope reading) | 2.15.15, Definition 2.47 (pp. 111-113) |
...
| Pretraining exposure is disclosed on a four-level ladder rather than settled by a terminology ban | synthesized from the source's single remark | 2.5.1 (p. 37) |
