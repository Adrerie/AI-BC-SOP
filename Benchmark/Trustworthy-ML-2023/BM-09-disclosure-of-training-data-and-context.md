...
- **Unit-of-analysis sweep**: where the task admits meaningful item-, document- and collection-level
  membership questions, report those units separately rather than pooling them. The 2026 course cites
  a modern-LLM setting in which sentence-level inference is near chance while collection-level inference
  is substantially stronger; treat that result as motivation for checking the unit of analysis, not as
  the universal shape that every model family should exhibit.
...
- **Baseline-provenance stress**: shadow or surrogate models used to build an attack classifier must be
  trained on data whose membership labels and examples are disjoint from the target evaluation set. If
  target membership information leaks into surrogate construction, the ordinary benchmark result is
  invalid and must be rerun. A deliberately stronger-information or oracle diagnostic may be shown only
  as a separate labeled row; it is not a repaired version of the leaked benchmark result.
...
- `auroc` for H-membership, with the positive class declared as *membership* and the score orientation
  stated; random ranking = 0.5. Use `tnr_at_high_tpr` where the claim is about an operating point rather
  than about ranking.
- **H-emission rate**: valid probes whose outputs satisfy the preregistered exact/near-match rule divided
  by valid probes, with the matching rule, match-length/edit-distance threshold and probe budget reported
  in the same table. Exact-match and near-match rates must remain separate if both are used.
- For H-channel, always report the **intermediate-channel disclosure rate**, the **final-answer disclosure
  rate**, and `disclosure_gap` = intermediate minus final over the same items and labeling rule. Larger
  gap means more disclosure through the exposed channel; the two raw rates are mandatory because the
  same zero gap can mean both channels are silent or both disclose heavily.
...
`aupr` with membership positive and its no-skill reference equal to the member prevalence, because
membership sets are often imbalanced; exact/near-match counts and match-length distributions for
H-emission; per-axis slopes
...
Report per unit of analysis and never across units. Where item- and collection-level membership are both
meaningful, report them separately and interpret each on its own terms rather than treating either as
an expected universal winner. Attach a
...
- A random AUROC reference is 0.5; the `aupr` no-skill reference is the member prevalence. Neither is a
  guarantee of safety: an attack can be weak today because better attacks exist tomorrow.
...
analysis, lists model size, sequence repetition and prompt length as axes along which memorisation was reported
in the taught examples, states differential-privacy budgets together with a utility trade-off in the
cited setting, composes them across steps, and contrasts the privacy-norm behavior of a model's intermediate
...
**repository conventions** added to make the session's claims executable as one benchmark. The score orientation, AUROC=0.5 random-ranking reference, AUPR member-prevalence reference and the
bootstrap requirement follow the package's existing metric discipline rather than the course. Nothing in this file is book-derived, and no
