...
3. Classify the intended claim into exactly one generalization type (ID / cross-domain /
   cross-bias / adversarial / other-name-it). If more than one applies, split the project into one
   sub-question per type; do not blend them into a single headline number.
...
- [ ] **One-type rule**: exactly one generalization type is claimed per reported headline number.
...
- [ ] **Pretraining audit**: for every pretrained component, name the corpus and check whether any
      evaluation class, task, or near-duplicate of it occurs there. If it does, the word "zero-shot"
